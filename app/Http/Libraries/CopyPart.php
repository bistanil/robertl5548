...
Class CopyPart{
	...
	public function part($item)
	{			
		$manufacturer = $this->getManufacturer($item->manufacturer);
		if ($manufacturer!=FALSE)
		{			
			$oldManufacturer = $this->getManufacturer($item->tdmanufacturer);
			if ($oldManufacturer != FALSE)
			{				
				$oldPart = CatalogProduct::join('part_codes', function ($join) use ($item) { 
                                        $join->on('catalog_products.id', '=', 'part_codes.part_id')
                                             ->where('part_codes.code', 'LIKE', preg_replace("/[^a-zA-Z0-9]+/","",$item->tdpartcode));
                                })
							->where('manufacturer_id', $oldManufacturer->id)
                ...
                {
                	$proceed = false;
					$part = CatalogProduct::whereManufacturer_id($manufacturer->id)->whereCode($item->partcode)->get()->first();
					if ($part == null)
					{
						$part = new CatalogProduct;
						$proceed = true;
					} else {
						if ($part->td_id  == 0) $proceed = true;
					}
					if ($proceed)
					{
						$part->slug = str_slug($item->title.'-'.$manufacturer->title.'-'.$item->partcode, '-');
						$part->manufacturer_id = $manufacturer->id;
						$part->code = $item->partcode;
						if ($item->title != '') $part->title = $item->title;
						else $part->title = $oldPart->title;
						$part->meta_title = $item->title.' | Cod '.$item->partcode;
						$part->meta_keywords = $item->title.' | Cod '.$item->partcode;
						$part->meta_description = $item->title.' | Cod '.$item->partcode;	
						...
						$part->type = 'new-am-part';
						$part->search_code = preg_replace("/[^a-zA-Z0-9]+/","",$part->code);
						if ($item->content != '') $part->content = $item->content;
						else $part->content = $oldPart->content;
			...
			} else File::append('public/files/import/excelPartsImportLog.txt', 'Manufacturer '.$item->tdmanufacturer.' not found!'."\n");							
		} else File::append('public/files/import/excelPartsImportLog.txt', 'Manufacturer '.$item->tdmanufacturer.' not found!'."\n");				
	}

	public function manufacturer($title)
	{		
		$manufacturer = new Manufacturer;
		$manufacturer = $manufacturer->bySlug(str_slug($title, '-'));
		return $manufacturer;
	}
