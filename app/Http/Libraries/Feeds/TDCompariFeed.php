...
Class TDCompariFeed{
  ...
  {         
    ...
    {      
      ...
      $productUrl = route('front-part', ['slug' => $product->slug]);
      if ($category != null)
      {
          $writer->insertOne([$product->manufacturer->title, $title, $category, $productUrl, $price, $part_id, $image, $description, 20, $product->code]);                
      }
    }
      ...
      $fileName = str_slug($this->request['file_name'], '-');
        ...
        if(! File::exists($file_path)){
            $excelFile = Excel::create($fileName, function ($csv) use($fileName)
            {
                ...
                $csv->setCreator('AutoPiesa')->setCompany('Auto Piesa.');
