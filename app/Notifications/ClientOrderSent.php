...
class ClientOrderSent extends Notification
{
    use Queueable;
    ...
    {
        return (new MailMessage)->markdown('mail.order.clientOrderSentMessage', ['order' => $this->order])
                                ->from(config('mail.defaultEmail'), config('mail.from.name'))
                                ->subject(trans('notification/order.newOrderSubject', ['orderNumber' => $this->order->id]))
                                ->greeting(trans('notification/order.greeting').' '.$this->order->client_name)
                                ->line(trans('notification/order.line1', ['orderNumber' => $this->order->id]))                                
