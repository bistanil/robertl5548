...
class ClientOrderUpdated extends Notification
{
    use Queueable;
    ...
    {
        return (new MailMessage)->markdown('mail.order.clientOrderUpdatedMessage', ['order' => $this->order])
                                ->from(config('mail.defaultEmail'), config('mail.from.name'))
                                ->subject(trans('notification/order.updateOrderSubject', ['orderNumber' => $this->order->id]))
                                ->greeting(trans('notification/order.greeting').' '.$this->order->client_name)
                                ->line(trans('notification/order.updateOrderLine1', ['orderNumber' => $this->order->id]))                                
